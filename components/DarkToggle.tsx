...
const DarkToggle: FC<DarkToggleProps> = ({ isDark, setIsDark }) => {
    return (
        <AnimatePresence>
            <motion.div
                className="w-14 h-14 z-50 bg-neutral-50 dark:bg-neutral-900 flex items-center justify-center cursor-pointer"
                onClick={() => setIsDark(!isDark)}
                key={isDark.toString()}
                initial={{ scale: 0, rotate: 0, borderRadius: "50%" }}
                animate={{ scale: 1, rotate: -360, borderRadius: "16px" }}
                whileTap={{ scale: 0.95, rotate: 0, borderRadius: "50%" }}
